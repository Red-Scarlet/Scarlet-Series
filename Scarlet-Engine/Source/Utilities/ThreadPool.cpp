...
namespace Scarlet {
	...
	{
        SCARLET_CORE_TRACE("[THREADPOOL] Num. Threads: " + ToString(m_NumThreads));
	}
    ...
    Scope<ThreadPool> ThreadPool::Create(const uint32& _Threads)
    {
        return CreateScope<ThreadPool>(_Threads);
    }
}